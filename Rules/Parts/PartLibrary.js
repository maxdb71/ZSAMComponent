...
import pageCaptionCount from '../../../SAPAssetManager/Rules/Common/PageCaptionCount';
import isSerialized from '../../../SAPAssetManager/Rules/Parts/Issue/SerialParts/SerialPartsAreAllowed';
...
// ENEL Customization
// Added some methods + modified the following ones:
// - partCreateUpdateValidation
// - partIssueCreateUpdateValidation
// - partMovementLineItemCreateUpdateSetODataValue
...
export default class {
    ...
    static partCreateUpdateValidation(pageClientAPI) {
        ...
        // ENEL Customization: added following two validations
        validations.push(libThis.validateQuantityHasNoDecimal(pageClientAPI, dict)); //Code Correction COD0067197 (Jira ID ICMTANGOAMF10-14302)
        validations.push(libThis.validateQuantityLessThanlimit(pageClientAPI, dict)); //Code Correction COD0067197 (Jira ID ICMTANGOAMF10-14302)
        if (dict.TextItemSim && dict.TextItemSim.visible) {
     ...
     * Code Correction COD0067197 (Jira ID ICMTANGOAMF10-14302)
     */
    static validateQuantityHasNoDecimal(pageClientAPI, dict) {
        var quantityDecimalCheck = dict.QuantitySim.getValue();
        if (quantityDecimalCheck == Math.floor(quantityDecimalCheck)) {
            return Promise.resolve(true);
        } else {
            let message = pageClientAPI.localizeText('validate_decimal_point');
    ...
    /**
     * ENEL Customization
     * Quantity number length check
     * Code Correction COD0067197 (Jira ID ICMTANGOAMF10-14302)
     */
    static validateQuantityLessThanlimit(pageClientAPI, dict) {
        var quantityLengthCheck = dict.QuantitySim.getValue();
        if (quantityLengthCheck.toString().length <= 9) {
            return Promise.resolve(true);
        } else {
            let message = pageClientAPI.localizeText('validate_length_quantity');
            libCom.setInlineControlError(pageClientAPI, dict.QuantitySim, message);
            return Promise.reject();
        }
    }
    ...
    static partIssueCreateUpdateValidation(pageClientAPI) {
        ...
        libCom.setInlineControlErrorVisibility(dict.StorageLocationLstPkr, false);
        //Enel customiziations (two following lines)
        libCom.setInlineControlErrorVisibility(dict.BatchNumLstPkr, false);
        dict.BatchNumLstPkr.clearValidation();
        ...
        //Enel customiziations (two following validations)
        promises.push(libThis.validateQuantityHasNoDecimal(pageClientAPI, dict)); //Code Correction COD0067197 (Jira ID ICMTANGOAMF10-14302)
        promises.push(libThis.validateQuantityLessThanlimit(pageClientAPI, dict)); //Code Correction COD0067197 (Jira ID ICMTANGOAMF10-14302)
        ...
        //Enel customisation: added the evalValuationTypeIsMandatory and validateValuationTypeNotBlank before Promising All
        return libThis.evalValuationTypeIsMandatory(pageClientAPI, dict).then(result => {
            if (result) {
                promises.push(libThis.validateValuationTypeNotBlank(pageClientAPI, dict));
            }
    ...
    /**
    * ENEL Customization
    * Evaluates whether batch list picker - valuation type is mandatory
    */
    static evalValuationTypeIsMandatory(context, dict) {
        return context.read('/SAPAssetManager/Services/AssetManager.service', 'MaterialBatches', [], "$select=Batch&$filter=Plant eq '" + context.binding.Plant + "' and MaterialNum eq '" + context.binding.MaterialNum + "'")
            .then(result => {
     ...
     * Enel customisation for ValuationType
     */
    static validateValuationTypeNotBlank(pageClientAPI, dict) {
        if (!libThis.evalValuationTypeIsEmpty(dict)) {
            return Promise.resolve(true);
        } else {
            let message = pageClientAPI.localizeText('field_is_required');
            libCom.setInlineControlError(pageClientAPI, dict.BatchNumLstPkr, message);
            return Promise.reject();
        }
    }
    ...
    /**
    * ENEL Customization
    * Evaluates whether batch list picker - valuation type is empty
    */
    static evalValuationTypeIsEmpty(dict) {
        return (libVal.evalIsEmpty(libCom.getListMultiplePickerValue(dict.BatchNumLstPkr.getValue())));
    }

    /**
    * ENEL Customization
    * used in ParteCreate.action
    */
    static PartCreateUpdateValuationTypePkrValue(context) {
        let ZValuationTypeLstPkr = libCom.getTargetPathValue(context, '#Control:ValuationTypeLstPkr/#Value');
        return libCom.getListPickerValue(ZValuationTypeLstPkr);
    }

    /**
    * ENEL Customization
    * used in PartCreateUpdate.page
    ...
    * @param {*} context 
    */
    static partValuationTypeQueryOptions(context) {
        return "$orderby=Batch&$filter=MaterialNum eq ''";

        let query = "$orderby=Batch&$filter=MaterialNum eq ''";

        if (context.binding.Plant && context.binding.MaterialNum) {
            query = `$orderby=Batch&$filter=MaterialNum eq '${context.binding.MaterialNum}' and Plant eq '${context.binding.Plant}'`;
        }
        return query;
    }
    ...
    */
    static partMovementLineItemCreateUpdateSetODataValue(pageClientAPI, key, movementType) {
        ...
        switch (key) {
            ...
            //ENEL Customization    
            case 'Batch':
                return libCom.getListPickerValue(libCom.getFieldValue(pageClientAPI, 'BatchNumLstPkr', '', null, true));
            default:
                return '';
        }
    }
